 ...
 * ===========================================================
 * Documentation Statement: Asked the instructor a few questions. Referenced class materials.
 * =========================================================== */
...
void* SRTFcpu(void* param) {
    ...
    while (1) {
        ...
        if (p == NULL) {
            ...
            // the longest (qInsert always appends to the tail, so the head is
            // always the oldest arrival — that is the FIFO selection rule).
            int qLoc = qShortest(&(svars->readyQ));
            p = qRemove(&(svars->readyQ), qLoc);
...
void* PPcpu(void* param) {
    ...
    while (1) {
        ...
        if (p == NULL) {
            ...
            // the longest (qInsert always appends to the tail, so the head is
            // always the oldest arrival — that is the FIFO selection rule).
            int qLoc = qPriority(&(svars->readyQ));
            p = qRemove(&(svars->readyQ), qLoc);
